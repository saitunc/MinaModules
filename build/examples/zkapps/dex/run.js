...
async function main({ withVesting }) {
    ...
    tx = await Mina.transaction({
        ...
        await dex.supplyLiquidityBase(UInt64.from(10000), UInt64.from(10000));
    });
    await tx.prove();
    ...
    tx = await Mina.transaction(feePayer, async () => {
        AccountUpdate.fundNewAccount(feePayer);
        await tokenY.transfer(addresses.tokenY, addresses.tokenX, UInt64.MAXINT().sub(200000));
    });
    await tx.prove();
    ...
    await expect(async () => {
        tx = await Mina.transaction(addresses.tokenX, async () => {
            await dex.supplyLiquidityBase(UInt64.MAXINT().sub(200000), UInt64.MAXINT().sub(200000));
        });
        await tx.prove();
        tx.sign([keys.tokenX]);
        await tx.send();
