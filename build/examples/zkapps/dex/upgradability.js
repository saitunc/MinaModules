...
async function upgradeabilityTests({ withVesting }) {
    ...
    tx = await Mina.transaction({
        ...
        await tokenX.transfer(addresses.tokenX, feePayer, 10000);
        await tokenY.transfer(addresses.tokenY, feePayer, 10000);
    ...
    expect(Mina.getAccount(addresses.dex, tokenX.deriveTokenId()).zkapp
        ?.verificationKey?.data).toEqual(ModifiedDexTokenHolder._verificationKey?.data);
    expect(Mina.getAccount(addresses.dex, tokenY.deriveTokenId()).zkapp
        ?.verificationKey?.data).toEqual(ModifiedDexTokenHolder._verificationKey?.data);
    ...
    tx = await Mina.transaction({
        ...
        await modifiedDex.supplyLiquidityBase(UInt64.from(10000), UInt64.from(10000));
    });
    await tx.prove();
